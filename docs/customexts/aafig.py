...
class AafigDirective(directives.images.Image):
    ...
    def run(self):
        ...
        options_copy = list(self.options.items())
        for (k, v) in options_copy:
...
def render_aafig_images(app, doctree):
    ...
    for img in doctree.traverse(nodes.image):
        ...
        if format in format_map:
            ...
            app.builder.warn(f'unsupported builder format "{format}", please '
...
def render_aafigure(app, text, options):
    ...
    if app.builder.format == 'html':
        ...
        if app.builder.format != 'latex':
            app.builder.warn(f'aafig: the builder format {app.builder.format} is not officially '
